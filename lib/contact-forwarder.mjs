...
export function createForwarder(env = process.env, fetcher = fetch, log = console.error) {
  ...
  async function redis(command) {
    const url = new URL(env.UPSTASH_REDIS_REST_URL || env.KV_REST_API_URL || 'https://invalid.local');
    const redisToken = env.UPSTASH_REDIS_REST_TOKEN || env.KV_REST_API_TOKEN;
    if (url.protocol !== 'https:' || !url.hostname.endsWith('.upstash.io') || url.username || url.password || !redisToken) throw new Error('queue_configuration');
